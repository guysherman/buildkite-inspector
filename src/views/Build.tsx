...
import { Fragment, FC } from '@guysherman/treecat';
...
export const Build: FC<BuildProps> = (props: BuildProps) => {
  ...
  return (
    <Fragment>
      <text top={index} left={0} width={1} style={{ fg: 'black', bg: color }}>
        {'\uE0B0'}
      </text>
      <text top={index} left={1} width={BRANCH_CELL_LENGTH} style={{ fg: 'black', bg: color }}>
        {center(`\uE0A0 ${branchText}`, BRANCH_CELL_LENGTH)}
      </text>
      <text top={index} left={14} width={MESSAGE_CELL_LENGTH} style={{ fg: color, bg: 'black' }}>
        {center(messageText, MESSAGE_CELL_LENGTH)}
      </text>
    </Fragment>
  );
};
